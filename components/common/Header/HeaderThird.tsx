import React from "react";
import ProductCard from "../UiKit/ProductCard";
import Link from "next/link";

function HeaderThird(props) {
  return (
    <section className="header-categories">
      <div className="header-categories__container">
        <menu className="header-categories__menu header-menu">
          <ul className="header-menu__list">
            <li className="header-menu__list-item">
              <a href="" className="header-menu__link">
                Apple
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <path
                    d="M14.1666 7.64168C14.0104 7.48647 13.7992 7.39935 13.5791 7.39935C13.3589 7.39935 13.1477 7.48647 12.9916 7.64168L9.99992 10.5917L7.04992 7.64168C6.89378 7.48647 6.68257 7.39935 6.46242 7.39935C6.24226 7.39935 6.03105 7.48647 5.87492 7.64168C5.79681 7.71915 5.73481 7.81132 5.69251 7.91287C5.6502 8.01442 5.62842 8.12334 5.62842 8.23335C5.62842 8.34336 5.6502 8.45228 5.69251 8.55383C5.73481 8.65538 5.79681 8.74754 5.87492 8.82501L9.40825 12.3583C9.48572 12.4365 9.57789 12.4984 9.67944 12.5408C9.78098 12.5831 9.88991 12.6048 9.99992 12.6048C10.1099 12.6048 10.2188 12.5831 10.3204 12.5408C10.4219 12.4984 10.5141 12.4365 10.5916 12.3583L14.1666 8.82501C14.2447 8.74754 14.3067 8.65538 14.349 8.55383C14.3913 8.45228 14.4131 8.34336 14.4131 8.23335C14.4131 8.12334 14.3913 8.01442 14.349 7.91287C14.3067 7.81132 14.2447 7.71915 14.1666 7.64168Z"
                    fill="white"
                    fillOpacity="0.5"
                  />
                </svg>
              </a>
              <menu className="header-menu__list-submenu header-submenu">
                <ul className="header-submenu__list">
                  <li className="header-submenu__list-item">
                    <div className="header-submenu__link">
                      <a href="" className="header-submenu__link-title">
                        Macbook Pro 14
                      </a>
                      <a href="" className="header-submenu__link-subtitle">
                        В категории Apple Macbook Air 13 (M1 2022)
                      </a>
                    </div>
                    <div className="header-submenu__results-box">
                      <div className="header-submenu__results">
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://100pristavok.ru/upload/iblock/bd8/bd8c92c56764ceaa1c5e25ccd03abfba.jpg"
                            }
                            name={"Playstation 5"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://avatars.mds.yandex.net/i?id=a7d7f9ef8c31c6c2ebc30fe55f5a8ab0dd97bed5-7716203-images-thumbs&n=13"
                            }
                            name={"Apple Imac / 6 gb / 12 gb / 1024 TB"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                      </div>
                      <Link className="header-submenu__results-link" href={""}>
                        Смотреть все товары
                      </Link>
                    </div>
                  </li>
                </ul>
              </menu>
            </li>
            <li className="header-menu__list-item">
              <a href="" className="header-menu__link">
                Samsung
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <path
                    d="M14.1666 7.64168C14.0104 7.48647 13.7992 7.39935 13.5791 7.39935C13.3589 7.39935 13.1477 7.48647 12.9916 7.64168L9.99992 10.5917L7.04992 7.64168C6.89378 7.48647 6.68257 7.39935 6.46242 7.39935C6.24226 7.39935 6.03105 7.48647 5.87492 7.64168C5.79681 7.71915 5.73481 7.81132 5.69251 7.91287C5.6502 8.01442 5.62842 8.12334 5.62842 8.23335C5.62842 8.34336 5.6502 8.45228 5.69251 8.55383C5.73481 8.65538 5.79681 8.74754 5.87492 8.82501L9.40825 12.3583C9.48572 12.4365 9.57789 12.4984 9.67944 12.5408C9.78098 12.5831 9.88991 12.6048 9.99992 12.6048C10.1099 12.6048 10.2188 12.5831 10.3204 12.5408C10.4219 12.4984 10.5141 12.4365 10.5916 12.3583L14.1666 8.82501C14.2447 8.74754 14.3067 8.65538 14.349 8.55383C14.3913 8.45228 14.4131 8.34336 14.4131 8.23335C14.4131 8.12334 14.3913 8.01442 14.349 7.91287C14.3067 7.81132 14.2447 7.71915 14.1666 7.64168Z"
                    fill="white"
                    fillOpacity="0.5"
                  />
                </svg>
              </a>
              <menu className="header-menu__list-submenu header-submenu">
                <ul className="header-submenu__list">
                  <li className="header-submenu__list-item">
                    <div className="header-submenu__link">
                      <a href="" className="header-submenu__link-title">
                        Macbook Pro 14
                      </a>
                      <a href="" className="header-submenu__link-subtitle">
                        В категории Apple Macbook Air 13 (M1 2022)
                      </a>
                    </div>
                    <div className="header-submenu__results-box">
                      <div className="header-submenu__results">
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://100pristavok.ru/upload/iblock/bd8/bd8c92c56764ceaa1c5e25ccd03abfba.jpg"
                            }
                            name={"Playstation 5"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://avatars.mds.yandex.net/i?id=a7d7f9ef8c31c6c2ebc30fe55f5a8ab0dd97bed5-7716203-images-thumbs&n=13"
                            }
                            name={"Apple Imac / 6 gb / 12 gb / 1024 TB"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                      </div>
                      <Link className="header-submenu__results-link" href={""}>
                        Смотреть все товары
                      </Link>
                    </div>
                  </li>
                </ul>
              </menu>
            </li>
            <li className="header-menu__list-item">
              <a href="" className="header-menu__link">
                Xiaomi
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <path
                    d="M14.1666 7.64168C14.0104 7.48647 13.7992 7.39935 13.5791 7.39935C13.3589 7.39935 13.1477 7.48647 12.9916 7.64168L9.99992 10.5917L7.04992 7.64168C6.89378 7.48647 6.68257 7.39935 6.46242 7.39935C6.24226 7.39935 6.03105 7.48647 5.87492 7.64168C5.79681 7.71915 5.73481 7.81132 5.69251 7.91287C5.6502 8.01442 5.62842 8.12334 5.62842 8.23335C5.62842 8.34336 5.6502 8.45228 5.69251 8.55383C5.73481 8.65538 5.79681 8.74754 5.87492 8.82501L9.40825 12.3583C9.48572 12.4365 9.57789 12.4984 9.67944 12.5408C9.78098 12.5831 9.88991 12.6048 9.99992 12.6048C10.1099 12.6048 10.2188 12.5831 10.3204 12.5408C10.4219 12.4984 10.5141 12.4365 10.5916 12.3583L14.1666 8.82501C14.2447 8.74754 14.3067 8.65538 14.349 8.55383C14.3913 8.45228 14.4131 8.34336 14.4131 8.23335C14.4131 8.12334 14.3913 8.01442 14.349 7.91287C14.3067 7.81132 14.2447 7.71915 14.1666 7.64168Z"
                    fill="white"
                    fillOpacity="0.5"
                  />
                </svg>
              </a>
              <menu className="header-menu__list-submenu header-submenu">
                <ul className="header-submenu__list">
                  <li className="header-submenu__list-item">
                    <div className="header-submenu__link">
                      <a href="" className="header-submenu__link-title">
                        Macbook Pro 14
                      </a>
                      <a href="" className="header-submenu__link-subtitle">
                        В категории Apple Macbook Air 13 (M1 2022)
                      </a>
                    </div>
                    <div className="header-submenu__results-box">
                      <div className="header-submenu__results">
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://100pristavok.ru/upload/iblock/bd8/bd8c92c56764ceaa1c5e25ccd03abfba.jpg"
                            }
                            name={"Playstation 5"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://avatars.mds.yandex.net/i?id=a7d7f9ef8c31c6c2ebc30fe55f5a8ab0dd97bed5-7716203-images-thumbs&n=13"
                            }
                            name={"Apple Imac / 6 gb / 12 gb / 1024 TB"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                      </div>
                      <Link className="header-submenu__results-link" href={""}>
                        Смотреть все товары
                      </Link>
                    </div>
                  </li>
                </ul>
              </menu>
            </li>
            <li className="header-menu__list-item">
              <a href="" className="header-menu__link">
                Смартфоны
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <path
                    d="M14.1666 7.64168C14.0104 7.48647 13.7992 7.39935 13.5791 7.39935C13.3589 7.39935 13.1477 7.48647 12.9916 7.64168L9.99992 10.5917L7.04992 7.64168C6.89378 7.48647 6.68257 7.39935 6.46242 7.39935C6.24226 7.39935 6.03105 7.48647 5.87492 7.64168C5.79681 7.71915 5.73481 7.81132 5.69251 7.91287C5.6502 8.01442 5.62842 8.12334 5.62842 8.23335C5.62842 8.34336 5.6502 8.45228 5.69251 8.55383C5.73481 8.65538 5.79681 8.74754 5.87492 8.82501L9.40825 12.3583C9.48572 12.4365 9.57789 12.4984 9.67944 12.5408C9.78098 12.5831 9.88991 12.6048 9.99992 12.6048C10.1099 12.6048 10.2188 12.5831 10.3204 12.5408C10.4219 12.4984 10.5141 12.4365 10.5916 12.3583L14.1666 8.82501C14.2447 8.74754 14.3067 8.65538 14.349 8.55383C14.3913 8.45228 14.4131 8.34336 14.4131 8.23335C14.4131 8.12334 14.3913 8.01442 14.349 7.91287C14.3067 7.81132 14.2447 7.71915 14.1666 7.64168Z"
                    fill="white"
                    fillOpacity="0.5"
                  />
                </svg>
              </a>
              <menu className="header-menu__list-submenu header-submenu">
                <ul className="header-submenu__list">
                  <li className="header-submenu__list-item">
                    <div className="header-submenu__link">
                      <a href="" className="header-submenu__link-title">
                        Macbook Pro 14
                      </a>
                      <a href="" className="header-submenu__link-subtitle">
                        В категории Apple Macbook Air 13 (M1 2022)
                      </a>
                    </div>
                    <div className="header-submenu__results-box">
                      <div className="header-submenu__results">
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://100pristavok.ru/upload/iblock/bd8/bd8c92c56764ceaa1c5e25ccd03abfba.jpg"
                            }
                            name={"Playstation 5"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                        <div className="header-submenu__result">
                          <ProductCard
                            image={
                              "https://avatars.mds.yandex.net/i?id=a7d7f9ef8c31c6c2ebc30fe55f5a8ab0dd97bed5-7716203-images-thumbs&n=13"
                            }
                            name={"Apple Imac / 6 gb / 12 gb / 1024 TB"}
                            price={850000}
                            mode={"light"}
                          />
                        </div>
                      </div>
                      <Link className="header-submenu__results-link" href={""}>
                        Смотреть все товары
                      </Link>
                    </div>
                  </li>
                </ul>
              </menu>
            </li>
          </ul>
        </menu>
      </div>
    </section>
  );
}

export default HeaderThird;
