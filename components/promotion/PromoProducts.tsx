...
function PromoProducts() {
  ...
  /* Эффекты */
  useEffect(() => {
    getClientDataFromAPI<ICategory[]>(
  ...
  useEffect(() => {
    if (!!router.query.categories__in) {
      getClientDataFromAPI<IPagination<IProduct>>(
        `/api/content/benefits/${router.query.link}/products/?categories__in=${router.query.categories__in}`,
        setProducts,
        request
      );
    } else {
